...
export function DriverDetailsTabs({
  ...
  const docs = useMemo(
    () =>
      driver.documentsDetail.map((d) => ({
        ...d,
        status: docOverrides[d.id] ?? d.status,
      })),
  ...
  const delSlice = filteredDeliveries.slice(
    (delPage - 1) * pageSize,
    delPage * pageSize,
  );
          ...
          <Card className="rounded-xl border-[#EEE7DF] shadow-soft lg:col-span-2">
            ...
            <CardContent className="space-y-2 text-sm text-muted-foreground">
              <p>
                ...
                · {driver.assignedRegion} · Success{" "}
                <span className="text-primary font-medium">{successRate}%</span>
              </p>
              <p>
                Wallet {formatMoney(driver.walletBalance)} · Avg{" "}
              ...
              <ul className="space-y-1 text-sm">
                <li className="flex gap-2">
                  <Check
                    className={cn(
                      "h-4 w-4",
                      driver.verificationSummary.identity
                        ? "text-emerald-600"
                        : "opacity-30",
                    )}
                  />
                ...
                <li className="flex gap-2">
                  <Check
                    className={cn(
                      "h-4 w-4",
                      driver.verificationSummary.vehicle
                        ? "text-emerald-600"
                        : "opacity-30",
                    )}
                  />
                ...
                <li className="flex gap-2">
                  <Check
                    className={cn(
                      "h-4 w-4",
                      driver.verificationSummary.insurance
                        ? "text-emerald-600"
                        : "opacity-30",
                    )}
                  />
          ...
          <Card className="rounded-xl border-primary/25 bg-primary/[0.04]">
            ...
            <CardContent className="text-sm">
              ...
              <div className="text-muted-foreground">
                {driver.currentDelivery.pickup} →{" "}
                {driver.currentDelivery.dropoff}
              </div>
            </CardContent>
          </Card>
        )}
      ...
      <TabsContent value="earnings" className="mt-0 space-y-4">
        <div className="flex flex-wrap justify-between gap-2">
          <p className="text-sm text-muted-foreground">
            Earnings & payouts (demo)
          </p>
          <Button
            variant="outline"
            size="sm"
            className="rounded-xl"
            onClick={onDownloadEarningsReport}
          >
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              ...
              size="sm"
              className="rounded-xl"
              onClick={onDownloadDeliveriesExport}
            >
              <Download className="h-4 w-4" />
              CSV
            </Button>
            ...
            <Table>
              ...
              <TableBody>
                {delSlice.map((row) => (
                  <MotionRow
                    key={row.id}
                    whileHover={{ backgroundColor: "rgba(137,81,41,0.06)" }}
                  >
                    ...
                    <TableCell className="max-w-35 truncate text-xs">
                      {row.pickup}
                    </TableCell>
                    <TableCell className="max-w-35 truncate text-xs">
                      {row.dropoff}
                    ...
                    <TableCell>
                      <Badge
                        variant={deliveryBadgeVariant(row.status)}
      ...
      <TabsContent value="documents" className="mt-0">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {docs.map((doc) => (
            <Card
              key={doc.id}
              className="overflow-hidden rounded-xl border-[#EEE7DF] shadow-soft"
            >
              ...
              <CardContent className="space-y-3 p-0">
                ...
                <div className="flex gap-2 px-4 pb-4">
                  <Button
                    ...
                    onClick={() =>
                      setDocOverrides((o) => ({ ...o, [doc.id]: "approved" }))
                    }
                  >
                    Approve
                  </Button>
                  <Button
                    ...
                    onClick={() =>
                      setDocOverrides((o) => ({ ...o, [doc.id]: "rejected" }))
                    }
                  >
                    Reject
                  </Button>
