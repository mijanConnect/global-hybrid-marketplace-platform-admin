...
export default function ProductsPage() {
  ...
  useEffect(() => {
    const onRename = (e: Event) => {
      ...
      setProducts((prev) =>
        prev.map((p) =>
          p.category === detail.from ? { ...p, category: detail.to } : p,
        ),
      );
    };
  ...
  const stats = useMemo(() => {
    ...
    const revenue = products.reduce(
      (sum, p) => sum + p.totalSales * p.price,
      0,
    );
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((p) => {
      ...
      const matchesOwnerType =
        ownerType === "all" || p.productOwnerType === ownerType;
      const matchesCountry = country === "all" || p.country === country;
      const matchesMin =
        min === undefined || (!Number.isNaN(min) && p.price >= min);
      const matchesMax =
        max === undefined || (!Number.isNaN(max) && p.price <= max);
      return (
        matchesTab &&
        matchesQuery &&
        matchesCategory &&
  ...
  function applyBulkCategory(nextCategory: string) {
    ...
    setProducts((prev) =>
      prev.map((p) =>
        selectedIds.has(p.id) ? { ...p, category: nextCategory } : p,
      ),
    );
    setSelectedIds(new Set());
  }
    ...
    setProducts((prev) =>
      prev.map((p) => {
        ...
        if (p.status === "inactive")
          return { ...p, status: p.stock === 0 ? "out_of_stock" : "active" };
        return p;
      }),
    );
  }
            ...
            >
              ...
              <TabsContent value={tab} className="mt-3 space-y-3">
                ...
                <ProductTable
                  ...
                  onDelete={(p) =>
                    setConfirm({ kind: "delete_one", productId: p.id })
                  }
